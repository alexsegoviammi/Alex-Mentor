...
document.addEventListener('DOMContentLoaded', () => {
  ...
  elements.chatMessages.addEventListener('click', (e) => {
    ...
    if (actionBtn && actionHandlers[actionBtn.dataset.action]) {
      actionHandlers[actionBtn.dataset.action]();
    }
  });
...
async function handleMessageProcessing(message) {
	ui.showTyping();
	const isGeneratingPlan = message
		.toLowerCase()
		.includes("generar plan completo");
	...
	try {
		...
		const pdfYaLlego =
			data.pdfGenerated === true || (data.pdfUrl && data.pdfUrl.length > 0);

		if (isGeneratingPlan && !state.isPolling && !pdfYaLlego) {
			console.log(
				"El PDF aún no llega. Iniciando búsqueda en segundo plano..."
			);
			startPDFPolling();
		} else if (pdfYaLlego) {
			console.log(
				"¡El PDF llegó directo con la respuesta! No hace falta buscarlo."
			);
		}
	} catch (error) {
		ui.hideTyping();
...
async function startPDFPolling() {
	...
	const poll = async () => {
		...
		if (attempts % 4 === 0) {
			// Cada minuto
			ui.showAIMessage(
				`Generando tu plan... ${Math.min(80 + attempts * 2, 95)}% completado.`
			);
		}
		setTimeout(poll, CONFIG.pollingInterval);
	};
...
function selectQuickReply(reply) {
	if (reply === "Generar plan completo") {
		...
		ui.removeQuickReplies();
		ui.showAIMessage(
			"¡Perfecto! Estoy generando tu plan de negocio completo. Esto tomará unos momentos..."
		);
		ui.markSectionCompleted(6);
		ui.activateSection("section-final");
		handleMessageProcessing("Generar plan completo");
...
async function promptEmailAndSend() {
	const email = prompt(
		"¿A qué dirección de email quieres enviar tu plan?\n\nEjemplo: nombre@gmail.com"
	);
	if (!email) return;
	...
	try {
		...
		ui.showError(
			"No se pudo enviar el email. Puedes descargar el PDF directamente."
		);
	}
}
...
function askQuestions() {
  ...
  \n\nPuedes preguntarme sobre:\n• Detalles de financiación disponible\n• Estrategias de marketing para tu cliente objetivo\n• Análisis de competencia en tu sector\n• Proyecciones financieras más detalladas\n• Pasos legales para constituir tu empresa\n• Contactos y recursos en Gran Canaria\n\n¿Qué te gustaría saber?`);
	elements.messageInput.placeholder =
		"Haz tu pregunta sobre el plan de negocio...";
	elements.messageInput.focus();
}
...
document.addEventListener("DOMContentLoaded", () => {
	...
	elements.chatMessages.addEventListener("click", (e) => {
		...
		const actionBtn = e.target.closest(".action-btn");
		if (actionBtn && actionHandlers[actionBtn.dataset.action]) {
			// CORRECCIÓN: Ejecutamos la función correctamente
			actionHandlers[actionBtn.dataset.action]();
		}
	});
